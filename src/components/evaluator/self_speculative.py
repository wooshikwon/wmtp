...
class SelfSpeculativeEvaluator(BaseComponent):
    ...
    def _speculative_decode_step(
        self, model: torch.nn.Module, input_ids: torch.Tensor, draft_length: int = 4
        ...
        with torch.no_grad():
            ...
            for h in range(min(heads, draft_length)):
                ...
                head_probs.append(
                    token_prob.item() if batch_size == 1 else token_prob.mean().item()
                )
    ...
    def _measure_acceptance_rate(
        self, model: torch.nn.Module, prompts: list[str], tokenizer: Any
    ) -> dict[str, Any]:
        """
        여러 시퀀스에 대한 수락률 측정.
        ...
        with Progress(
            ...
            task = progress.add_task(
                "[cyan]Measuring acceptance rates...", total=len(prompts)
            )
        ...
        avg_accepted_length = (
            np.mean([s["avg_accepted"] for s in sequence_stats])
            if sequence_stats
            else 0
        )
    ...
    def run(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        if not model or not tokenizer:
            raise ValueError(
                "Model and tokenizer are required for self-speculative evaluation"
            )
        ...
        console.print(
            "[bold cyan]Starting Self-Speculative Decoding Evaluation[/bold cyan]"
        )
    ...
    def _print_results(self, acceptance_metrics: dict, speedup_metrics: dict):
        ...
        table.add_row(
            ...
            f"{acceptance_metrics['overall_acceptance_rate']:.2%}",
        )
        table.add_row(
            "Average Accepted Length",
            f"{acceptance_metrics['average_accepted_length']:.2f} tokens",
        )
        table.add_row(
            "Theoretical Speedup", f"{acceptance_metrics['effective_speedup']:.2f}x"
        )
        ...
        if speedup_metrics:
            table.add_row(
                "Measured Speedup", f"{speedup_metrics['measured_speedup']:.2f}x"
            )
            table.add_row(
                "Speculative Time", f"{speedup_metrics['speculative_time']:.2f}s"
            )
