...
class InferenceSpeedEvaluator(BaseComponent):
    ...
    def run(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        if not model or not tokenizer:
            raise ValueError("Model and tokenizer are required for inference speed evaluation")
        ...
        # 전체 평균 계산
        all_mtp_speedups = [v["mtp_speedup"] for v in results["speedup_metrics"].values()]
        all_spec_speedups = [v["speculative_speedup"] for v in results["speedup_metrics"].values()]
        ...
        if results["summary"]["avg_mtp_speedup"] < 1.0:
            console.print("[red]Warning: MTP is slower than NTP. Check model implementation.[/red]")
        else:
            console.print(f"[green]Average MTP speedup: {results['summary']['avg_mtp_speedup']:.2f}x[/green]")

        return {
            "metrics": {
                "inference_speed_mtp": results["summary"]["avg_mtp_speedup"],
                "inference_speed_speculative": results["summary"]["avg_speculative_speedup"],
    ...
    def _benchmark_speculative_decoding(
        ...
        # 더미 입력 생성
        input_ids = torch.randint(
            0, tokenizer.vocab_size, (batch_size, 32),  # 시작 시퀀스
            device=self.device
        )
                ...
                while tokens_generated < seq_len:
                    ...
                    if hasattr(outputs, 'prediction_logits'):
                        ...
                        if proposed_tokens:
                            ...
                            current_input = torch.cat([current_input, new_tokens], dim=-1)
                            tokens_generated += len(proposed_tokens)
                        else:
                            # 폴백: NTP 방식으로 1개 토큰 생성
                            next_token = outputs.prediction_logits[:, -1, 0, :].argmax(dim=-1, keepdim=True)
